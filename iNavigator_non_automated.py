...
def gazeDetectionLR(eye_points, facial_landmarks):
    ...
    _, threshold_eye = cv2.threshold(gray_eye, 70, 255,
                                     cv2.THRESH_BINARY)  # creating a threshold
...
def gazeDetectionUD(eye_points, facial_landmarks):
    ...
    gray_eye = cv2.cvtColor(eye, cv2.COLOR_BGR2GRAY)  # making gray scale
    _, threshold_eye = cv2.threshold(gray_eye, 70, 255,
                                     cv2.THRESH_BINARY)  # creating a threshold dont know exactly what it mean
    h, w = threshold_eye.shape
...
def eye_movement_detectLR(net_gaze_ratio):
    ...
    if net_gaze_ratio>=0 and net_gaze_ratio<=1.1:
        return "Right"
    elif net_gaze_ratio>1.1 and net_gaze_ratio<=2.55:
        return "Center"
    else:
        return "Left"
...
while (True):
    ...
    # BLINK DETECTION
        ...
        if left_ratio >=6:
            cv2.putText(frame, "left blink", (50,200), font, 1, (255,0,0), 3 )
            pyautogui.click()
        if right_ratio >=6:
